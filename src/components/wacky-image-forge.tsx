...
import { Sparkles, Wand2, Download, Repeat, Loader2, Cat, Dog, Bird, Bug, Bot, Ghost, Gem, Mountain, Zap, BookOpen, Brush, Code, Bike, Ship, Castle, Cloud, Leaf, Droplets, Flame, Music, Palette, Camera, Square, Glasses, Aperture, VenetianMask } from 'lucide-react';
...
const keywordCategories: { [key: string]: { color: string; textColor: string; keywords: { [key: string]: ReactNode } } } = {
  ...
  Actions: {
    ...
    textColor: 'text-white',
    keywords: {
      'eating spaghetti': <SpaghettiIcon />,
  ...
  Settings: {
    ...
    textColor: 'text-white',
    keywords: {
      'on Saturn': <SaturnIcon />,
      'in a jungle': <Leaf />,
      'underwater': <Droplets />,
      'in a castle': <Castle />,
      'on a mountain top': <Mountain />,
      'in a neon-lit city': <Zap />,
      'inside a volcano': <Flame />,
      'at a disco': <Music />,
      'in a library of lost books': <BookOpen />,
      'on a pirate ship': <Ship />,
    }
  },
  Styles: {
    color: 'bg-[#4A90E2]',
  ...
  const handleChaos = () => {
    startTransition(async () => {
      ...
      if (error || !prompt) {
        ...
        const promptParts = prompt.replace(/, in /g, ',').split(',');
        const cleanPromptParts = promptParts.map(p => p.replace('A ', '').replace(' style.', '').trim());
        
        const animal = cleanPromptParts[0] || '';
        const action = cleanPromptParts[1] || '';
        const setting = cleanPromptParts[2] || '';
        const style = cleanPromptParts[3] || '';

        if(animal) newSelected.set('Animals', animal);
        if(action) newSelected.set('Actions', action);
        if(setting) newSelected.set('Settings', setting);
        if(style) newSelected.set('Styles', style);
